import { LoginForm, LoginResponse, UserDispatch } from "../../types/user";
import api from "../../utils/api";

export const login = (creds: LoginForm) => async (dispatch: UserDispatch) => {
    dispatch({ type: "LOGIN_START"});
    try {
        const response = await api().post<LoginResponse>("/user/login",creds);
...
export const isLoggedIn = () => async (dispatch: UserDispatch) => {
    ...
    try {
      const response = await api().post<LoginResponse>("/user/is_logged_in");
